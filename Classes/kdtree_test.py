...
if __name__ == "__main__":
    ...
    points = []
    for i in range(1000):
        p = kdtr.Point(random.gauss(dd[0]/2, dd[0]/8), random.gauss(dd[1]/2, dd[1]/8))
        quadtree.insert(p)
        points.append(p)
    ...
    while not game_exit:
        ...
        if one_pressed:
            width = 100
            height = 100
            rectangle = kdtr.Boundary(target[0] - width/2, target[1] - height/2, target[0] + width/2, target[1] + height/2)
        ...
        if three_pressed:
            p = kdtr.Point(target[0], target[1])
            quadtree.insert(p)
            points.append(p)
            three_pressed = False
